...
def reqister():
    ...
    if form.validate_on_submit():
        ...
        send_email(email=form.email.data,
                   text=f"Дорогой друг, {user.name}!\n Мы благодарим тебя за интерес к нашему сайту.",
                   zaglav='Спасибо за регистрацию')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.password.data):
            ...
            send_email(email=form.email.data,
                       text=f"Дорогой друг, {user.name}!\n Кто-то вошёл в Ваш аккаунт! Если это не Вы, то срочно смените пароль!",
                       zaglav='Уведомление безопасности')
            return redirect("/menu")
        send_email(email=form.email.data,
                   text=f"Дорогой друг, {user.name}!\n Кто-то пытался войти в аккаунт использовая неверный пароль.",
                   zaglav='Уведомление безопасности')
